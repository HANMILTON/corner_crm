  ...
  <div v-for="(v,index) in data" class="col-sm-4">
    <div class="border border-box">
        ...
        <pk-input sm="12" label="推荐理由">
          <textarea class="form-control" v-model="v.recommend_words" rows="4"></textarea>
        </pk-input>      
      </div> 
  ...
  userShopService
    ...
    methods:{
      ...
      updateFood(data,type,index){
        if(type == "del"){
          if(data.id){
            userShopService.shopRecommendDelete(data.id)
          ...
          userShopService.shopRecommendSubmit(data)
          .then(res => this.updateList(res,index))
        }else{
          data.recommend_id = 0
